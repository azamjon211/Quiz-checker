...
@section('title', 'Test Kaliti Kiritish')
@section('breadcrumb', 'Admin / Test Kalitlari / Yangi')

@section('content')
<div class="max-w-3xl" x-data="quizBuilder()" x-init="init()">
    <form method="POST" action="{{ route('admin.quizzes.store') }}" @submit="prepareForm">
        @csrf
        ...
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 mb-4">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-slate-300 text-sm font-medium mb-2">Test nomi <span class="text-rose-400">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" required
                           placeholder="Masalan: Matematika — 7-sinf variantı"
                ...
                <div>
                    <label class="block text-slate-300 text-sm font-medium mb-2">Tavsif (ixtiyoriy)</label>
                    <textarea name="description" rows="2"
                              placeholder="Quiz haqida qisqacha..."
                              class="w-full bg-slate-900 border border-slate-600 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition placeholder-slate-500 resize-none">{{ old('description') }}</textarea>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-slate-300 text-sm">Faol (foydalanuvchilar ko'ra oladi)</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 mb-4">
            ...
            <div class="flex items-center gap-4">
                <input type="number" x-model="count" name="question_count" min="1" max="200" required
                       @change="generateQuestions"
                       class="w-32 bg-slate-900 border border-slate-600 text-white rounded-xl px-4 py-3 text-sm text-center font-bold text-lg focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                <div class="flex gap-2">
                    @foreach([10,20,30,40,50] as $n)
                    <button type="button" @click="count={{ $n }}; generateQuestions()"
                            class="px-3 py-2 bg-slate-700 hover:bg-indigo-500/20 border border-slate-600 hover:border-indigo-500/40 text-slate-300 hover:text-indigo-400 rounded-lg text-sm font-medium transition-all">
                        {{ $n }}
                    </button>
                    @endforeach
                </div>
        ...
        {{-- Answers grid --}}
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 mb-6" x-show="questions.length > 0">
            <div class="flex items-center justify-between mb-5">
                <h2 class="text-white font-semibold flex items-center gap-2">
                    <span class="w-6 h-6 bg-indigo-500/20 border border-indigo-500/30 rounded-lg flex items-center justify-center text-indigo-400 text-xs font-bold">3</span>
                    Javob kalitini kiriting (to'g'ri javoblar)
                </h2>
                <div class="flex gap-2">
        ...
        <div class="flex gap-3">
            <button type="submit"
                    class="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40">
                Test kalitini saqlash
            </button>
...
function quizBuilder() {
    return {
        ...
        init() {
            this.count = {{ old('question_count', 10) }};
            this.generateQuestions();
        },

        generateQuestions() {
            const n = parseInt(this.count) || 0;
            const prev = [...this.questions];
            this.questions = [];
            for (let i = 1; i <= n; i++) {
                const old = prev.find(q => q.number === i);
                this.questions.push({ number: i, answer: old ? old.answer : '' });
            }
        },
